...
def calculate_diff(latents1, latents2):

    diff = np.sum(np.abs((latents1 - latents2)), axis=0)
    return diff
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if save_checkpoint:
                    ...
                    if i == 9:
                        final_mask = np.zeros(diff.shape)
                        for temp_mask in self.mask_list:
                            final_mask += (temp_mask - np.min(temp_mask)) / (np.max(temp_mask) - np.min(temp_mask))
                        ...
                        kernel = np.ones((2, 2))
                        final_mask = cv.erode(final_mask, kernel, 1)
                        final_mask = cv.dilate(final_mask, kernel, 1)
                        mask = np.equal(final_mask, 255).astype(np.uint8)
