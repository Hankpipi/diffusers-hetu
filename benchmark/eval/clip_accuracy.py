...
################ Cosine Similarity ################
embeddings_1 = torch.load("texts_edit.pth", map_location=device)
...
################ CLIP ACC ################
embeddings_1 = torch.load("texts_edit.pth", map_location=device)
embeddings_2 = torch.load("hetu_edit.pth", map_location=device)
...
with torch.no_grad():
    ...
    acc = (similarity < similarity2)
    print((acc.trace() / len(acc)).item())